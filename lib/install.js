...
/**
 * Read the config file and return the entries map regardless of root key name.
...
function readConfig(configPath) {
  ...
  const entries = Object.values(json)[0];
 ...
 *
 * @param {string} name
 * @param {{ repo: string, version: string, asset: string, dest?: string }} entry
...
async function installOne(name, entry, baseDir, lockPath, clean) {
  const { repo, version, asset: assetTemplate, dest } = entry;
  ...
  if (!clean && fs.existsSync(destDir) && locked === version) {
  ...
  try {
    ...
    const structure = extractZip(zipPath, name, destDir);
    info(`  ↳ Detected structure: ${structure}`);

    writeLock(lockPath, name, version);
...
module.exports = { install, installOne, readConfig };
